...
if($_POST['id'] != "" ){ $id  = $_POST['id']+1; } else{ $id  = ''; } 
...
if($action=='action_change')
{
    ...
    else
    {
        if($type =='DCFD')
        {
            $coaSampleArray = getAllRecords($connection, 'tbl_sub_assembly_sub_component', array('assembly_id_fk' => $edit_table_id,'type' => 'DCFD'));
        }
        else
        {
            $coaSampleArray = getAllRecords($connection, 'tbl_sub_assembly_sub_component', array('assembly_id_fk' => $edit_table_id,'type' => 'assembly'));
        }
        $data = array();
...
{ 
    $select_assembly = "SELECT * FROM tbl_subsu_assembly";
...
?>
    <div class="remove_<?php echo $id;  ?> form-inline" style="margin-top: 10px; margin-left:0px;">
        <label>
            <select name="sub_component_id[]" data-id="<?php echo $id; ?>" id="subcomp_<?php echo $id; ?>" class="form-control sub_component_id_onchange 123456" placeholder="Select Sub Component" style="<?php echo $margin; ?>">
                <option value=""></option>
                <?php 
...
{   
    ...
    <br>
        <label>
            <select name="sub_component_id[]" data-id="<?php echo $id; ?>" id="subcomp_<?php echo $id; ?>" class="form-control sub_component_id_onchange" placeholder="Select Sub Component" <?php echo $disabled; ?> style="<?php echo $margin; ?>">
                <option value=""></option>
                <?php 
                while ($rows1 = $queryVendor->fetch_assoc()) 
                { ?>
