...
// include('selected_details.php');
  ...
  $select_vendor  = "SELECT * FROM tbl_sub_component where is_active !='inactive'";
  $queryVendor    = selectQuery($connection, $select_vendor);
  $isAddEdit      = (!isset($_GET['id']) || $subComponentArray->num_rows==0) ? 'add' : 'edit';

  $selectQ = "SELECT * FROM tbl_subsu_assembly where id='".$_GET['id']."'";
      ...
      <section class="content">
        <form class="form" name="form_product " id="form_product" method="POST" action="insert-update.php" enctype="multipart/form-data">
            <div class="card">
              ...
              <h3 class="card-title"><i class="fas fa-briefcase"></i>&nbsp; <?php echo isset($_GET['id']) ? "SubSU Number - Edit": "SubSU Number - Add"; ?></h3>
              <div class="card-tools">
                <a href="./subsu_assembly.php" class="btn btn-primary btn-sm"><i class="fas fa-arrow-left"></i>&nbsp;Back</a>
              </div>
              </div>
              <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-6">
                      <div class="form-group">
                        <label class="control-label">Image Upload</label>:
                          ...
                          <input type="hidden" name="type" value="SUB_ASSEBLY" />
                      </div>
                    </div>
                </div>
              </div>
            ...
            <div class="card">
                <div class="card-header">
                  <h3 class="card-title">Vendor Part (Subcomponents)</h3>            
                </div>
                <div class="card-body sub-component-div">
                            ...
                            <span>Vendor Part</span>
                              <select data-id="0"  name='sub_component_id[]' id="subcomp_0" class="form-control sub_component_id_onchange" placeholder="Select Sub Component" <?php echo $disabled; ?>>
                                  ...
                                  <?php 
                                     ...
                                     { ?>
                                         <option value="<?php echo $rows1['id']; ?>"><?php echo $rows1['sub_copoment_name']; ?></option>
                                     <?php }?>
                              </select> 
                          </label>
                            <label>
                  ...
                  <div class="row">
                      <div class="col-md-12">
                      <a class="btn btn-primary extra-fields-customer1 float-right" style="color:white; margin-top: -40px; margin-right: 15px; <?php if(!isset($_SESSION['LOGIN']['user_id'])){ echo "display:none";} ?>"><i class="fa fa-plus"></i></a>
            ...
            <div class="card-footer">
              ...
              <a class="btn btn-default white mr-0" href="./subsu_assembly.php"><i class="ft ft-x-circle"></i> Cancel</a>
...
$(document).ready(function() 
{
    ...
    var total_data = 0;
    $.ajax({
            url:"add-edit-subassembly_add_more.php",  
            type: "post", 
            dataType: "json",
            async: false,
                ...
                for(var i=0; i<result.length;i++)
                  {
                    ...
                    $(".view_data_"+i).append('<a class="form-control btn btnprimary" href="./add-edit-subcomponent.php?id='+result[i]+'" target="_blank">View</a>')
                  }  
              },700);
            }
        });
