...
$materialIds = ($id!="") ? getAllIds($connection, 'parent_material', 'material_id', array('parent_id'=>$id,'check_flag'=>"sub_component")) : array();
$manufacturerIds = ($id!="") ? getAllIds($connection, 'parent_manufacturer', 'manufacturer_id', array('parent_id'=>$id,'check_flag'=>"sub_component")) : array();
$productIds = ($id!="") ? getAllIds($connection, 'parent_product', 'product_id', array('parent_id'=>$id,'check_flag'=>"sub_component")) : array();
$processIds = ($id!="") ? getAllIds($connection, 'parent_process', 'process_id', array('parent_id'=>$id,'check_flag'=>"sub_component")) : array();
$equipmentIds = ($id!="") ? getAllIds($connection, 'parent_equipment', 'equipment_id', array('parent_id'=>$id,'check_flag'=>"sub_component")) : array();
...
$sub_copoment_name = isset($_POST['sub_copoment_name']) ? $_POST['sub_copoment_name'] : "";
...
$leachable_or_validation = isset($_POST['leachable_or_validation']) ? $_POST['leachable_or_validation'] : array();
...
$errMsg = "";
$formAction = isset($_GET['id']) ? "add-edit-subcomponent.php?id=".$_GET['id'] : "add-edit-subcomponent.php";
$status = ($id=="") ? "added" : "updated";


function add_multiple_children($connection, $table, $fieldName, $aid, $dataArray) {
	$deletePreviousData = deleteRecord($connection, $table, array('parent_id'=>$aid,'check_flag'=>'sub_component'));
		if(count($dataArray) > 0){
			foreach ($dataArray as $value) {
				$data = array('parent_id'=>$aid, $fieldName=>$value,'check_flag'=>'sub_component');
...
if(isset($_POST['submit'])){
	$isvalid = empty($sub_copoment_name) ?  false : true;
	
	if($isvalid){
	// $targetDir = '../assets/uploads/drawing-sub-component';
	// $drawingFile = $_FILES['drawing_of_sub_component'];
	// $fileName = (isset($drawingFile["name"]) && $drawingFile["name"]!='') ? time(). '_' .$drawingFile["name"] : "";
	// $drawingUpload = ($fileName!="") ? fileUpload($targetDir, $_FILES['drawing_of_sub_component'], $fileName, 1000000, array("png", "jpg", "jpeg")) : false;

	if (($_FILES['drawing_of_sub_component']['name']!="")){
		$target_dir = 'assets/uploads/drawing-sub-component/';
		$file = $_FILES['drawing_of_sub_component']['name'];
		$path = pathinfo($file);
		$filename = $path['filename'];
		$ext = $path['extension'];
		$filename = time().'.'.$ext;
		$temp_name = $_FILES['drawing_of_sub_component']['tmp_name'];
	...
	$dataArray = array(
		'sub_copoment_name' => $sub_copoment_name,
	...
	if (($_FILES['drawing_of_sub_component']['name']!="")){
		$dataArray['drawing_of_sub_component'] = $filename;
	}
	...
	// Upload Pdf start
	$deletePreviousCoa = deleteRecord($connection, 'tbl_sub_component_coa_sample', array('sub_component_id_fk'=>$aid));

	if(count($savedCoa) > 0){
		$i=0;
		foreach ($savedCoa as $value) {
			$insertSavedcoa = ($value!="") ? insertRecord($connection, 'tbl_sub_component_coa_sample', array('coa_sample_file'=>$value, 'other_location'=>$savedOtherLocation[$i], 'data_id_fk'=>$savedDataIdFk[$i], 'sub_component_id_fk'=> $aid)) : "";
			$i++;
		}
	}

	$targetDirCoa = '../assets/uploads/coa-sample-sub-component';
	$coaSamplesFileArray = array();
		...
		if($coaSamples!="" && count($coaSamples['name']) > 0){
			for ($x=0; $x < count($coaSamples['name']); $x++) {
				...
				$fileNameCoa = (isset($coaSamples['name'][$x]) && $coaSamples['name'][$x]!='') ? $coaSamples['name'][$x] : "";
				...
				$coa = ($fileNameCoa!="") ? insertRecord($connection, 'tbl_sub_component_coa_sample', array('coa_sample_file'=>$fileNameCoa, 'other_location'=>$otherLocation[$x], 'data_id_fk'=>$dataIdFk[$x], 'sub_component_id_fk'=> $aid)) : "";
...
// Upload Pdf end	
	if($addData){
	     ...
	     $_SESSION['toast_list']['message'] = "Sub Component ".$status." successfully.";
		header('location:'.SITE_URL.'sub-components.php'); exit;
	}else{
...
if(isset($_GET['id'])){
	...
	$coaSampleArray = getAllRecords($connection, 'tbl_sub_component_coa_sample', array('sub_component_id_fk' => $_GET['id']));
	
	while ($row = $selectRecord->fetch_assoc()) { 
		$id = $row["id"];
		$sub_copoment_name = $row["sub_copoment_name"];
		$internal_part_no = $row["internal_part_no"];
		$drawing_of_sub_component = $row["drawing_of_sub_component"];
...
$typeDropdown = getDropdown($connection, 'subcomponent_type', 'id', 'name', array('is_active'=>1), $type_id);
