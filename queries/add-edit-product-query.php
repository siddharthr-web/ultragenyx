...
$dcfd_id_fk_array = isset($_POST['dcfd_id_fk']) ? $_POST['dcfd_id_fk'] : array();
...
$dcfdIds = ($id!="") ? getAllIds($connection, 'parent_dcdf', 'dcfd_id', array('parent_id'=>$id,'check_flag'=>"assembly")) : array();
// $dcfdIds =  array();
...
if(isset($_POST['submit'])){
   ...
   if($nameCount!=0){
     $errMsg =  "Assembly name already exist.";
     $isvalid = false;
   }

  if($isvalid){
	if (($_FILES['drawing_of_assembly']['name']!="")){
		$target_dir = 'assets/uploads/drawing-assembly/';
		$file = $_FILES['drawing_of_assembly']['name'];
		$path = pathinfo($file);
		$filename = $path['filename'];
		$ext = $path['extension'];
		$filename = time().'.'.$ext;
		$temp_name = $_FILES['drawing_of_assembly']['tmp_name'];
	...
	$insertDCFD =  (isset($_POST['dcfd_id_fk']) && count($_POST['dcfd_id_fk']) > 0) ? add_multiple_children($connection, 'parent_dcdf','dcfd_id', $aid, $_POST['dcfd_id_fk']) : add_multiple_children($connection, 'parent_dcdf','dcfd_id', $aid, array());
	...
	if(isset($_POST['sub_component_id']))
	{
		...
		if(count($_POST['sub_component_id']) > 0)
		{
			...
			$subComponentSaveArray = array_unique($_POST['sub_component_id']);

			// echo "<pre>";
			// print_r($_POST['sub_component_id']);
			// exit;

			
			foreach ($subComponentSaveArray as $value) 
			{
				if($value!=""){
					$data = array('assembly_id_fk'=>$aid, 'sub_component_id_fk'=>$value,'type'=>'vendor');
...
// $subComponentDropdownArray = getDropdown($connection, 'tbl_sub_component', 'id', 'sub_copoment_name', array('is_active!'=>"Inactive"), '');


// $subComponentDropdownArray = getDropdownArray($connection, 'tbl_sub_component', 'id', 'sub_copoment_name', array('is_active!'=>"Inactive"));
