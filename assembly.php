...
$selectQ = "SELECT * FROM tbl_su_assembly  ORDER BY id DESC";
      ...
      <section class="content">
        <div class="card">
          <div class="card-header">
            <h3 class="card-title"><i class="fas fa-tasks"></i>&nbsp; SU Numbers Management  </h3>
            <div class="card-tools">
              <?php if(isset($_SESSION['LOGIN']['user_id'])){ ?>
                <a href="./add-edit-assembly.php" id="Add" class="btn btn-primary btn-sm" onclick="GoToAdd()" style="color: white;"> <i class="fa fa-plus"></i> Add </a>
              <?php } ?>
            </div>
          </div>
          <!-- Bootstrap 3 table -->
                      ...
                      <div class="float-right search btn-group">
                        <form method="post" id="search_data_live">
                            <div class="input-group">
                              ...
                              <input type="hidden" name="type_search" value="search_data_assembly"/>
                              <a href="./assembly.php<?php echo isset($_GET['sub_component_id']) ? '?sub_component_id='.$_GET['sub_component_id'] : ''; ?>" class="btn btn-primary" type="button" name="clearSearch" id="clearSearch" title="Clear Search"> <i class="fa fa-undo"></i> </a>
                            </div>
                        </form>
                      </div>
                    </div>
                        ...
                        while ($row = $queryResult->fetch_assoc()) 
                        {
                         ...
                         ?>
                          <tr data-index="0" data-uniqueid="2">
                            ...
                            <td style="width:50% ;"><a style="cursor: pointer; width:30%" href="./add-edit-assembly.php?id=<?php echo $row['id']; ?>" title="Edit"><?php if(!empty($row['name'])){ echo $row['name']; } if(!empty($row['materials'])){ echo $row['materials']; }  ?></a></td>
                            <?php if(isset($_SESSION['LOGIN']['user_id'])){ ?>
